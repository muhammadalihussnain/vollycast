 ...
 * Collapsible. Start/stop YouTube or Facebook broadcast.
 */
        ...
        <>
          {!isLive ? (
                ...
                {(['youtube', 'facebook', 'custom'] as PlatformType[]).map((p) => (
                  <button key={p} onClick={() => { setPlatform(p); }}
                    className={[
                      ...
                      platform === p ? 'bg-brand-accent text-brand-dark' : 'bg-brand-card text-slate-400 hover:text-white',
                    ].join(' ')}>
                    {PLATFORM_LABELS[p]}
                  </button>
                ))}
            ...
            <div className="space-y-3">
              ...
              <button onClick={() => { void handleStop(); }} disabled={loading}
